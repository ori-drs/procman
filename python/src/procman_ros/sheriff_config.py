    ...
    def next_token(self):
        ...
        if c == '"':
            ...
            while True:
                ...
                if c == "\\":
                    c = self._unescape(self._next_char())
                elif not c or c == '"':
        ...
        raise ParseError(
            self.line_num, self.line_pos, self.line_buf, None, "Invalid character"
        )
...
class GroupNode:
    ...
    def to_config_string(self, indent=0):
        ...
        if self.name == "":
            assert indent == 0
            val = "\n".join(
                [group.to_config_string(0) for group in list(self.subgroups.values())]
            )
            val = (
                val
            ...
            val = val + "\n".join(
                [
                    group.to_config_string(indent + 1)
                    for group in list(self.subgroups.values())
                ]
            )
            val = val + "\n".join(
...
class Parser:
    ...
    def _parse_command_param_list(self, cmd):
        ...
        if attrib_name == "stop_signal" and attrib_val < 1:
            ...
            self._fail(
                "Invalid value specified for command attribute 'stop_time_allwoed'"
            )
    ...
    def _parse_start_stop_restart_action(self, action_type):
        ...
        if self._eat_token(TokEndStatement):
            return StartStopRestartActionNode(action_type, ident_type, ident, None)
        self._expect_identifier("wait", "Expected ';' or 'wait'")
        wait_status = self._parse_string_one_of(["running", "stopped"])
        self._eat_token_or_fail(TokEndStatement, "Expected ';'")
        return StartStopRestartActionNode(action_type, ident_type, ident, wait_status)
