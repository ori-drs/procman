...
bool Procman::KillCommand(ProcmanCommandPtr cmd, int signum) {
  if (0 == cmd->Pid()) {
    printf("[DEBUG] [%s] has no PID.  not stopping (already dead)\n", cmd->ExecStr().c_str());
    return false;
  }
  ...
  for (int child_pid : descendants) {
    printf("[DEBUG] signal %d to descendant %d\n", signum, child_pid);
    ...
    for (ProcmanCommandPtr cmd : commands_) {
      ...
      for (int child_pid : cmd->descendants_to_kill_) {
        if(IsOrphanedChildOf(child_pid, pid)) {
          printf("[DEBUG] sending SIGKILL to orphan process %d\n", child_pid);
