...
from procman_msgs.msg import ProcmanCmd
from procman_msgs.msg import ProcmanDeputyInfo
from procman_msgs.msg import ProcmanOrders
from procman_msgs.msg import ProcmanCmdDesired
from procman_msgs.msg import ProcmanCmdStatus
from procman_msgs.msg import ProcmanDiscovery
...
class Command:
    ...
    def _update_from_cmd_info(self, cmd_msg):
        ...
        if (
            self._pid == 0
            and self._actual_runid == self._desired_runid
            and not self._auto_respawn
            and not self._force_quit
        ):
        ...
        if self._desired_runid != self._actual_runid and not self._force_quit:
            ...
            if self._pid > 0:
                ...
                if self._scheduled_for_removal:
                    ...
                    self._force_quit
                    and os.WIFSIGNALED(self._exit_code)
                    and os.WTERMSIG(self._exit_code)
                    in [signal.SIGTERM, signal.SIGINT, signal.SIGKILL]
                ):
                    return STOPPED_OK
                else:
...
class Deputy:
    ...
    def _owns_command(self, cmd_obj):
        return (
            cmd_obj._command_id in self._commands
            and self._commands[cmd_obj._command_id] is cmd_obj
        )
    ...
    def _update_from_deputy_info(self, dep_info_msg):
        ...
        can_safely_remove = [
            ...
            if cmd._scheduled_for_removal and cmd._command_id not in updated_ids
        ]
...
class Sheriff(Node):
    ...
    def __init__(self):
        ...
        self.info_sub = self.create_subscription(ProcmanDeputyInfo, "/procman/info", self._on_pmd_info, 10)
        self.orders_sub = self.create_subscription(ProcmanOrders, "/procman/orders", self._on_pmd_orders, 10)
        self.orders_pub = self.create_publisher(ProcmanOrders, "/procman/orders", 10)
        self.discover_pub = self.create_publisher(ProcmanDiscovery, "/procman/discover", 10)
        ...
        discover_msg.timestamp = self.get_clock().now().to_msg()
    ...
    def __command_status_changed(self, cmd_obj, old_status, new_status):
        self._queued_events.append(
            lambda listener: listener.command_status_changed(
    ...
    def __observer_status_changed(self, is_observer):
        self._queued_events.append(
            lambda listener: listener.observer_status_changed(is_observer)
        )
        self._condvar.notify()
    ...
    def _add_command(
        ...
        if not exec_str:
            raise ValueError("Invalid command {}".format(exec_str))
        if not command_id:
    ...
    def remove_empty_deputies(self):
        ...
        with self._lock:
            for deputy_id, deputy in list(self._deputies.items()):
                ...
                if not deputy._commands or all(
                    [cmd._scheduled_for_removal for cmd in cmds]
                ):
    ...
    def _add_commands_from_config(self, group_node, name_prefix):
        ...
        for subgroup in list(group_node.subgroups.values()):
            if group_node.name:
                self._add_commands_from_config(
                    subgroup, name_prefix + group_node.name + "/"
                )
            else:
            ...
            with self._lock:
                ...
                if now > next_send:
                    if not self._is_observer:
                        self._send_orders()
                    next_send = min(
                        time.time() + send_interval, next_send + send_interval
                    )
